	...
	<fieldset class="filters btn-toolbar">
		<?php if ($this->params->get('filter_field') != 'hide') :?>
			<div class="btn-group">
				...
				<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox form-control" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_WEBLINKS_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_WEBLINKS_FILTER_SEARCH_DESC'); ?>" />
			</div>
		<?php endif; ?>
				...
				<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
					...
					<?php if ($canEdit) : ?>
						<span class="list-edit pull-left width-50">
							<?php echo JHtml::_('icon.create_edit', $item, $params); ?>
						</span>
					<?php endif; ?>
					...
					<div class="list-title">
						...
						<div class="img-intro-<?php echo htmlspecialchars($imgfloat); ?> img-responsive"> <img
							<?php if ($images->image_first_caption):
								$captionClass = "";
								echo $captionClass="caption".' title="' .htmlspecialchars($images->image_first_caption) .'"';
							endif; ?>
							src="<?php echo htmlspecialchars($images->image_first); ?>" alt="<?php echo htmlspecialchars($images->image_first_alt); ?>" class="img-responsive <?php echo $captionClass;?>"/> </div>
						<?php endif; ?>
						...
						<?php if ($images->image_second_caption):
							$captionClass = "";
							echo $captionClass="caption".' title="' .htmlspecialchars($images->image_second_caption) .'"';
						endif; ?>
						src="<?php echo htmlspecialchars($images->image_second); ?>" alt="<?php echo htmlspecialchars($images->image_second_alt); ?>" class="img-responsive <?php echo $captionClass;?>"/> </div>
						<?php endif; ?>
