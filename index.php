...
if($isLoggedIn) 
{	
  $customUserFields = FieldsHelper::getFields('com_users.user', $user, true);
  $customUserFieldsRawValues = array_column($customUserFields, 'rawvalue', 'name'); 
  if (isset($customUserFieldsRawValues['avatar'])) 
  {
    $avatarField = json_decode($customUserFieldsRawValues["avatar"]);
    if ($avatarField->imagefile!='') 
    {
      $userPhoto=Uri::root().$avatarField->imagefile;
    } 
    else 
    {
...
<?php include "css.php" ?>
    ...
    {
			var v = 0;
			jQuery('.sidebar-left').children('.well').children().each(
        function() 
        {
...
<?php if ($renderModules and $hasDebug) : ?>
